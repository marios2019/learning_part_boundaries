...
Small edits by Marios Loizou
"""

import numpy as np
import tensorflow as tf
from local_frames import construct_local_frames, transform_neighbors, rotate_vectors
...
def get_edge_feature(point_cloud, nn_idx, use_local_frame=False, local_frame_data=None, add_normals=False, k=20):
	"""Construct edge feature for each point
	Args:
	  point_cloud: (batch_size, num_points, 1, num_dims)
	  nn_idx: (batch_size, num_points, k)
	  use_local_frame: bool
	  local_frame_data: tuple((batch_size, num_points, 3)x3)
	  add_normals: bool
	  k: int
	Returns:
	...
	if use_local_frame:
		assert(local_frame_data is not None)

		# Get central points xyz
		central_xyz = tf.slice(point_cloud, [0, 0, 0], [-1, -1, 3])
		# Get local frames transformation
		transformations = construct_local_frames(points=central_xyz, normals=local_frame_data[2], tangent_vec1=local_frame_data[0],
								   				 tangent_vec2=local_frame_data[1], return_R=add_normals)

		if add_normals:
			# Translate
			point_cloud_neighbors = point_cloud_neighbors - point_cloud_central
			# Get neighbors xyz
			neighbors_xyz = tf.slice(point_cloud_neighbors, [0,0,0,0], [-1,-1,-1,3])
			# Get neighbors normals
			neighbors_normals = tf.slice(point_cloud_neighbors, [0,0,0,3], [-1,-1,-1,3])
			# Rotate neighboring points and normals
			R = transformations[1]
			neighbors_xyz = rotate_vectors(vectors=neighbors_xyz, R=R)
			neighbors_normals = rotate_vectors(vectors=neighbors_normals, R=R)
			point_cloud_neighbors = tf.concat([neighbors_xyz, neighbors_normals], axis=-1)
		else:
			# Project neighboring points
			M = transformations[0]
			point_cloud_neighbors = transform_neighbors(neighbors=point_cloud_neighbors, M=M)
